...
func main() {
	...
	slopeValue := os.Getenv("SLOPE_VALUE")
	initialValue := os.Getenv("INITIAL_VALUE")
	log.Printf("batchNum: %v\n",batchNum)
	log.Printf("intervalTime: %v\n",intervalTime)
	log.Printf("slopeValue: %v\n",slopeValue)
	log.Printf("initialValue: %v\n",initialValue)
	...
	slope, err := strconv.Atoi(slopeValue)
	if err != nil {
		panic(err)
	}
	initial, err := strconv.Atoi(initialValue)
	if err != nil {
		panic(err)
	}
