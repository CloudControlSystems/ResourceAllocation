...
func writeDataInRedis(task WorkflowTask, redisServer string){
	...
	if task.TaskOrder == 0 {
		...
		//write worfklow task's data into Redis.
...
func clientTaskCreatePod(task WorkflowTask ,clientService *kubernetes.Clientset,
	...
	if thisTaskPodExist == true {
		...
		//invoke the resource allocation algorithm
        task.Cpu, task.Mem = resourceAllocateAlgorithm(task,redisIpPort)

		//write experimental data into /home/exp.txt.
		outData := "AllocationResource: " + task.TaskName + " : " + "CPU: " + strconv.Itoa(int(task.Cpu)) + ": Memory: "+ strconv.Itoa(int(task.Mem)) +"\n"
		experimentalDataObj.WriteString(outData)

		log.Printf("resourceAllocateAlgorithm: %dm, %dMi.\n",task.Cpu,task.Mem)
...
//Baseline-As long as one node can accommodate the task pod' request , the Baseline algorithm allocate resources.
func FcfsResourceAllocateAlgorithm(task WorkflowTask,redisIpPort string) (cpuNum uint64, memNum uint64) {
	...
	for {
		...
		//Obtain the largest remaining Cpu resources in the cluster nodes
		...
		if (nodeResidualResourceMaxCpu >= task.Cpu) && (nodeResidualResourceMaxMem >= task.Mem) {
...
func resourceAllocateAlgorithm(task WorkflowTask,redisIpPort string) (cpuNum uint64, memNum uint64) {
  ...
  for {
	  ...
	  log.Printf("accumulatedCpuFromRedis: %v,accumulatedMemFromRedis: %v\n",
	  ...
	  if (accumulatedCpuFromRedis != 0) && (accumulatedMemFromRedis != 0) {
		  ...
		  if (accumulatedCpuFromRedis <= cpuResidualResourceValue) &&
			  (accumulatedMemFromRedis <= memResidualResourceValue) {
			  if (task.Cpu < nodeResidualResourceMaxCpu) && (task.Mem < nodeResidualResourceMaxMem) {
				  //Create this task POD if the cluster resources are sufficient.
				  cpuNum = task.Cpu
				  memNum = task.Mem
			  ...
			  if (accumulatedCpuFromRedis > cpuResidualResourceValue) &&
				  ...
				  cpuNumTemp := task.Cpu * cpuResidualResourceValue / accumulatedCpuFromRedis

				  if (cpuNumTemp < nodeResidualResourceMaxCpu) && (task.Mem < nodeResidualResourceMaxMem) {
					  //Create this task POD if the cluster resources are sufficient.
					  cpuNum = cpuNumTemp
					  memNum = task.Mem
					  log.Printf("allocation pod's cpuNum: %dm, allocation pod's memNum: %dMi.\n", cpuNum, memNum)
				  ...
				  if (accumulatedCpuFromRedis < cpuResidualResourceValue) &&
					  ...
					  memNumTemp := task.Mem * memResidualResourceValue / accumulatedMemFromRedis
					  if (task.Cpu < nodeResidualResourceMaxCpu) && (memNumTemp < nodeResidualResourceMaxMem) {
						  //Create this task POD if the cluster resources are sufficient.
						  cpuNum = task.Cpu
						  memNum = memNumTemp
						  log.Printf("allocation pod's cpuNum: %dm, allocation pod's memNum: %dMi.\n", cpuNum, memNum)
					  ...
					  memNum = memNumTemp
					  log.Printf("allocation pod's cpuNum: %dm, allocation pod's memNum: %dMi.\n", cpuNum, memNum)
					//  if (cpuNumTemp < nodeResidualResourceMaxCpu) && (memNumTemp < nodeResidualResourceMaxMem) {
					//	  //Create this task POD if the cluster resources are sufficient.
					//	  cpuNum = cpuNumTemp
					//	  memNum = memNumTemp
					//	  log.Printf("allocation pod's cpuNum: %dm, allocation pod's memNum: %dMi.\n", cpuNum, memNum)
